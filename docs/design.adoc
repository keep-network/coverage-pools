...
ERC-20 token deposited in the pool by underwriters and accepted by the pool as
collateral. Collateral token positions of underwriters can be affected when the
Risk manager claims coverage. Token accepted as collateral needs to pass
