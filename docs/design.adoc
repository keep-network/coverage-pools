...
** `liquidationNotifierReward`: determines the amount of COV tokens which is
granted as reward for reporting about a deposit liquidation process.
* For `notifyLiquidated` rewards:
** `liquidatedNotifierReward`: determines the amount of COV tokens which is
granted as reward for reporting about a deposit being liquidated outside of the
coverage pool.
