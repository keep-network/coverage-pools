...
describe("EthAssetPool", () => {
  ...
  describe("deposit", () => {
    ...
    const depositedUnderwriter3 = to1e18(20)
      ...
      beforeEach(async () => {
        await ethAssetPool.connect(underwriter1).deposit({value: depositedUnderwriter1})
        await ethAssetPool.connect(underwriter2).deposit({value: depositedUnderwriter2})
        await ethAssetPool.connect(underwriter3).deposit({value: depositedUnderwriter3})
      })

      it("should mint underwriter tokens", async () => {
        expect(await underwriterToken.balanceOf(underwriter1.address))
        .to.be.equal(to1e18(100))
        expect(await underwriterToken.balanceOf(underwriter2.address))
        .to.be.equal(to1e18(300))
        expect(await underwriterToken.balanceOf(underwriter3.address))
